    ...
    convert: function (source) {
        ...
        snapshot.element.forEach(resource => {
            ...
            if(resource.base && resource.base.path && (resource.base.path.endsWith(".text") || resource.base.path.endsWith(".id"))){  
                ...
                reference = resource.contentReference.split(".").pop();
                prop = {
                    "type": "object",
                    "properties": {
                        "reference": {
            ...
            if (parts.length <= 2) {
                ...
                temp.forEach(x => {
                    if (currentSpot[x] == undefined && currentSpot.type==="object") {
                        currentSpot[x] = Object.assign({properties:{}}, currentSpot[x])
                    }
                    else {
                        currentSpot = currentSpot[x];
                    }
                ...
                if (!currentSpot.properties && currentSpot.type==="object") {
        ...
        switch (typeItem.code) {
            ...
            case "Reference":
                let reference = typeItem.profile ?
                    ...
                    : typeItem.targetProfile ? Array.isArray(typeItem.targetProfile) ?typeItem.targetProfile[0].split("/").pop() : typeItem.targetProfile.split("/").pop() : "anything";
                    reference.split(".").pop();
                let type = {
                    "type": "object",
                    "properties": {
