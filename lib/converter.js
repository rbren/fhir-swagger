...
module.exports = {
    convert: function (source) {
        ...
        snapshot.element.forEach(resource => {
            ...
            if (resource.base?.path.endsWith(".text")) {
                ...
                if (resource.contentReference.startsWith("#")) {
                    prop = JSON.parse(JSON.stringify(schema.properties[name]))
                }
            ...
            if (parts.length <= 2) {
                ...
                if(currentSpot.properties.items){
                    if(currentSpot.properties.items.oneOf){
                        currentSpot.properties.items.oneOf.push(prop)
                    }
                    else{
                        currentSpot.properties.items = prop
                    }
    ...
    getDefault: async function (version) {
        return await fetch("https://www.hl7.org/fhir/"+version+"/fhir.schema.json").then(res=>res.json())
    }
}
        ...
        switch (x.code) {
            ...
            case "Reference":
                let reference = resource.type[i].profile ?
                    ...
                    : resource.type[i].targetProfile ? resource.type[i].targetProfile.split("/").pop() : "anything";
                let type = {
                    "type": "object",
                    "properties": {
                        "reference": {
