...
Converter.convert = async function(baseURL, conf,options) {
  ...
  defaultSchemes= JSON.parse(JSON.stringify(defaultSchemes).replaceAll("#/definitions/","#/components/schemas/"))
  definitions=Object.assign(defaultSchemes.definitions, definitions);
  definitions= CleanupSwagger(definitions,swagger);
  swagger=oasReducer(swagger)
  return swagger;
}
