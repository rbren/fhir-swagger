...
const getType = require('./converter').getType
...
Converter.convert = async function (baseURL, conf, options) {
    ...
    resources.forEach(function (res) {
        ...
        if (interactions.indexOf('search-type') !== -1 && res.searchParam) {
            ...
            if (!Array.isArray(res.searchParam)) { //single searchParam
                res.searchParam = new Array(res.searchParam);
            }
    ...
    var operations = (!conf.rest.operation) ? [] : Array.isArray(conf.rest.operation) ? conf.rest.operation : [conf.rest.operation];

    // operations.forEach((x) => {
    //     if (x.instance === "true") {
    //         let opType = swagger.paths["/" + x.resource + "/{id}" + x.definition.reference] = {
    //             parameters: [
    //                 {
    //                     "in": "path",
    //                     "required": true,
    ...
    //             }
    //         }
    //         var parameters = (!x.parameters) ? [] : Array.isArray(x.parameters) ? x.parameters : [x.parameters];
    ...
    //                 opType.get.parameters.push(paramProp)
    //             } else {
    //                 let paramProp = {
    //                     type: getType(param.type, {})
    //                 };
    ...
    let keys = Object.keys(swagger.paths);
    let paths = {};
    keys.forEach((x) => {
        paths[x] = "*"
    })
